...
{
public:
  constexpr GpioBus(const std::array<GpioMask, N>& ms);
...
template<size_t N>
constexpr GpioBus<N>::GpioBus(const std::array<GpioMask, N>& ms) :  baseOffsets{0},
								   lsbIndexs{0},
								   widths{0},
...
template<size_t N>
constexpr bool GpioBus<N>::areMasksValid(const std::array<GpioMask, N>& ms)
{
...
template<size_t N>
ioportmask_t GpioBus<N>::readConf(size_t index) const
{
  if (index >= N)
    return USHRT_MAX;
